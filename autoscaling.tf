resource "aws_appautoscaling_target" "table_read" {
  count = var.create_table && var.autoscaling_enabled && var.autoscaling_read != null ? 1 : 0
...
resource "aws_appautoscaling_policy" "table_read_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = coalesce(var.autoscaling_read.scale_in_cooldown, var.autoscaling_defaults.scale_in_cooldown)
    scale_out_cooldown = coalesce(var.autoscaling_read.scale_out_cooldown, var.autoscaling_defaults.scale_out_cooldown)
    target_value       = coalesce(var.autoscaling_read.target_value, var.autoscaling_defaults.target_value)
  }
}

resource "aws_appautoscaling_target" "table_write" {
  count = var.create_table && var.autoscaling_enabled && var.autoscaling_write != null ? 1 : 0
...
resource "aws_appautoscaling_policy" "table_write_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = coalesce(var.autoscaling_write.scale_in_cooldown, var.autoscaling_defaults.scale_in_cooldown)
    scale_out_cooldown = coalesce(var.autoscaling_write.scale_out_cooldown, var.autoscaling_defaults.scale_out_cooldown)
    target_value       = coalesce(var.autoscaling_write.target_value, var.autoscaling_defaults.target_value)
  }
}
...
resource "aws_appautoscaling_policy" "index_read_policy" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    scale_in_cooldown  = coalesce(each.value.scale_in_cooldown, var.autoscaling_defaults.scale_in_cooldown)
    scale_out_cooldown = coalesce(each.value.scale_out_cooldown, var.autoscaling_defaults.scale_out_cooldown)
    target_value       = coalesce(each.value.target_value, var.autoscaling_defaults.target_value)
  }
}
